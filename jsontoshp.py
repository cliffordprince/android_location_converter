...
def make_reader(in_json):
    ...
    for item in json_data['locations']:
        date = item['timestamp'][:10]#datetime.datetime.fromtimestamp(int(item['timestamp'])/1000).strftime('%Y-%M-%D')
        time = item['timestamp'][11:19]#datetime.datetime.fromtimestamp(int(item['timestamp'])/1000).strftime('%H:%M:%S')
    ...
    # If there are any recorded activities in this entry, pick them apart by navigating through the json
        try:
            ...
            sub_date = datetime.datetime.fromtimestamp(int(item['activitys'][0]['timestamp'])/1000).strftime('%Y-%M-%D')
            sub_timestamp = datetime.datetime.fromtimestamp(int(item['activitys'][0]['timestamp'])/1000).strftime('%H:%M:%S')
        except:
            activities = None
            sub_date = None
            sub_timestamp = None

        yield [date, time, sub_date, sub_timestamp, longitude, latitude, accuracy, activities]
